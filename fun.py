...
class fun(commands.Cog):
    ...
    async def yui(self, ctx):
        ...
        yui_user = await guild.fetch_member(178752448809271296)
    ...
    async def avatar_cmd(ctx, self):
        member = guild.get_member(ctx.message.mentions[0].id if len(ctx.message.mentions) > 0 else ctx.author.id)
